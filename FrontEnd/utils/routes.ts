...
//done
  ...
  export const fetchUniqueEmails = async (email_id:string,sessionId:string) => {
    try {
...
export const chatWithChatBot = async (message: string, sessionId: string) => {
  try {
      const response = await backendAxios.post(`chatBot/chat/`, { message: message }, {
          headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${sessionId}`,
          },
      ...
      return { success: false, message: 'An error occurred while sending a message' };
  }
};
...
export const addGmailToken = async (token: string,sessionId:string) => {
  try {

      const response = await backendAxios.post(`accounts/gmailToken/`, { token: token }, {
          headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${sessionId}`,
          },
